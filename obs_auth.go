...
func getV2StringToSign(method, canonicalizeURL string, headers map[string][]string, isObs bool) string {
	...
	var query []string
	if !isSecurityToken {
		params := strings.Split(canonicalizeURL, "?")
		if len(params) > 1 {
...
func (o obsClient) doAuth(method, bucketName, objectKey string, params map[string]string,
	...
	isV4 := o.conf.signature == SignatureV4
	prepareHostAndDate(headers, hostName, isV4)

	if isAkSkEmpty {
		err = errors.New("No ak/sk provided, skip to construct authorization")
		return
	} else {
		ak := sh.accessKey
		sk := sh.securityKey
		...
		if isV4 {
			...
			ret := v4Auth(ak, sk, o.conf.region, method, canonicalizeURL, parsedRequestURL.RawQuery, headers)
			...
			ret := v2Auth(ak, sk, method, canonicalizeURL, headers, isObs)
...
func (o obsClient) doAuthTemporary(method, bucketName, objectKey string, params map[string]string,
	...
	if isAkSkEmpty {
		...
		if isV4 {
			...
			credential, scope := getCredential(sh.accessKey, o.conf.region, shortDate)
...
func getCredential(accessKey, region, shortDate string) (string, string) {
	scope := getScope(region, shortDate)

	return fmt.Sprintf("%s/%s", accessKey, scope), scope
}
...
func getV4StringToSign(method, canonicalizeURL, queryURL, scope, longDate, payload string, signedHeaders []string, headers map[string][]string) string {
	...
	canonicalRequest = append(canonicalRequest, canonicalizeURL)
	canonicalRequest = append(canonicalRequest, "\n")
...
// V4鉴权
func v4Auth(ak, sk, region, method, canonicalizeURL, queryURL string, headers map[string][]string) map[string]string {
	var t time.Time
	...
	stringToSign := getV4StringToSign(method, canonicalizeURL, queryURL, scope, longDate, payload, signedHeaders, _headers)
